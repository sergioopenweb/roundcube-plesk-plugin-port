...
class automatic_addressbook extends rcube_plugin
{
    ...
    /**
     * Safely decode an RFC822 address header on modern Roundcube.
     */
    private function decode_address_header($header_value, $charset)
    {
        if ($header_value === '') {
            return [];
        }

        return rcube_mime::decode_address_list($header_value, null, true, $charset ?: null);
    }
    ...
    public function init()
    {
        ...
        if (is_file($this->home . '/config/config.inc.php')) {
    ...
    public function register_recipients($p)
    {
        ...
        $all_recipients = array_merge(
            $this->decode_address_header((string) ($headers['To'] ?? ''), $charset),
            $this->decode_address_header((string) ($headers['Cc'] ?? ''), $charset),
            $this->decode_address_header((string) ($headers['Bcc'] ?? ''), $charset)
        );
        ...
        foreach ($all_recipients as $recipient) {
            ...
            if (!empty($recipient['mailto'])) {
                ...
                foreach ($book_types as $id) {
                    ...
                    if ($previous_entries && !empty($previous_entries->count)) {
                        break;
                    }
                }
    ...
    public function handle_doubles($args) 
    {
        ...
        if (($args['source'] ?? null) == $this->abook_id && !empty($args['id']) && $moveto) {
        ...
        if (($args['source'] ?? null) !== $this->abook_id) {
            $record = (array) ($args['record'] ?? []);
            $contact_emails = [];

            foreach (array('email:home', 'email:work', 'email:other') as $email_field) {
                if (!empty($record[$email_field]) && is_array($record[$email_field])) {
                    foreach ($record[$email_field] as $email) {
            ...
            if (!empty($record['email'])) {
                if (is_array($record['email'])) {
                    $contact_emails = array_merge($contact_emails, $record['email']);
                } else {
                    $contact_emails[] = $record['email'];
                }
            }
            ...
            if ($rcmail->task == "addressbook" &&
                $rcmail->action == "copy" &&
                ($_REQUEST['_source'] ?? null) == $this->abook_id) {
                $rcmail->output->command('parent.list_contacts');
            }
        }
