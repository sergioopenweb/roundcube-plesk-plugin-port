...
class libkolab extends rcube_plugin
{
    ...
    /**
     * Resolve a stylesheet path with fallbacks for manual Plesk installs.
     */
    private function stylesheet_path($local_file, $fallback_file = null)
    {
        $local_skin_path = $this->local_skin_path();
        $local_path = $local_skin_path . '/' . $local_file;

        if (is_file($this->home . '/' . $local_path)) {
            return $local_path;
        }

        if ($fallback_file && is_file($this->home . '/' . $fallback_file)) {
    ...
    public function init()
    {
        ...
        if (!empty($rcmail->output->type) && $rcmail->output->type == 'html') {
            ...
            $this->include_stylesheet($this->stylesheet_path('libkolab.css', 'skins/larry/libkolab.css'));
        }
...
*/
    ...
    public static function http_request($url = '', $method = 'GET', $config = [])
    {
        ...
        if (!empty(self::$http_requests[$key])) {
            ...
            $request->setHeader('user-agent', $_SERVER['HTTP_USER_AGENT'] ?? 'Roundcube/Kolab');
