...
class calendar_ui
{
    ...
    /**
     * Include a stylesheet with a larry fallback when Elastic assets are absent.
     */
    private function include_skin_stylesheet($filename)
    {
        $skin_path = $this->cal->local_skin_path();
        $local_path = $skin_path . '/' . $filename;

        if (is_file($this->cal->home . '/' . $local_path)) {
            $this->cal->include_stylesheet($local_path);
            return;
        }

        $fallback_path = 'skins/larry/' . $filename;

        if (is_file($this->cal->home . '/' . $fallback_path)) {
            $this->cal->include_stylesheet($fallback_path);
            return;
        }
    ...
    public function addCSS()
    {
        if (
            ...
            $this->include_skin_stylesheet('fullcalendar.css');
        }

        $this->include_skin_stylesheet('calendar.css');

        if ($this->rc->task == 'calendar' && $this->rc->action == 'print') {
            $this->include_skin_stylesheet('print.css');
        }
    }
